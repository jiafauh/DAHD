...
class CILSetTask:
        ...
        for class_n, videos in data.items():
            ...
            for video in videos:
                if self.is_activityNet:
                    ...
                    idx = None
                    import re
                    subpaths = re.split(r'[\\/]', video_name)
                    if len(subpaths) <= 2:
                        path_video = os.path.join(self.path_frames, class_n, video_name)
                    else:
                        video_name = subpaths[-1]
                        path_video = os.path.join(self.path_frames, class_n, video_name)
                    if not os.path.exists(path_video):
                        path_video = os.path.join(self.path_frames, video_name)
                    # # 在路径字符串中检测并移除重复的子路径
                    # normalized_path = path_video.replace('\\', os.sep).replace('/', os.sep)
                    # import re
                    # # 使用正则表达式匹配重复的子路径
                    # pattern = re.compile(r'({sep}[^{sep}]+)({sep}\1)+'.format(sep=re.escape(os.sep)))
                    # corrected_path = pattern.sub(r'\1', normalized_path)
                    # path_video = corrected_path.split(os.sep)
                    # unique_parts = []
                    # for part in path_video:
                    #     if unique_parts and unique_parts[-1] == part:
                    #         continue
                    #     unique_parts.append(part)
                    # path_video = os.sep.join(unique_parts)
                    frames = os.listdir(path_video)
    ...
    def __next__(self):
        ...
        if self.train_enable:
            ...
            comp_data = new_data
        current_task_dataset = TSNDataSet(self.path_frames, comp_data, self.num_frame_to_save, False, None,
                                          self.num_segments,
                                          self.new_length, self.modality, self.transform, self.random_shift,
                                          self.test_mode,
                                          self.remove_missing, self.dense_sample, self.twice_sample)
        len_data = len(current_task_dataset.video_list)
        # for i in range(len(current_task_dataset)):
        #     print(current_task_dataset[i])
        self.current_task_dataloader = DataLoader(current_task_dataset, batch_size=self.batch_size,
                                                  shuffle=self.shuffle,
                                                  num_workers=0, pin_memory=self.pin_memory,
                                                  drop_last=self.drop_last)
        # self.current_task_dataloader = DataLoader(current_task_dataset, batch_size=2,
        #                                           shuffle=self.shuffle,
        #                                           num_workers=self.num_workers, pin_memory=True,
        #                                           drop_last=self.drop_last)
        # for batch in self.current_task_dataloader:
        #     print(batch)
        self.current_task += 1
        if self.current_task < len(self.set_tasks):
            return comp_data.keys(), data, self.current_task_dataloader, len_data, len(
